...
def parse_arguments():
    ...
    parser.add_argument('--Data_limietd', action='store_true', default=False, 
                        help='Save training metrics to a JSON file')
    parser.add_argument('--fraction', type=float, default=1,
                         help='A float between 0 and 1 indicating the fraction of the data to retain.')
...
def main():
    ...
    retain_index = int(len(X_train) * args.fraction)

    X_train = X_train[:retain_index]
    X_train_with_estimate = X_train_with_estimate[:retain_index]
    y_train_actual = y_train_actual[:retain_index]
    y_train_estimated = y_train_estimated[:retain_index]
